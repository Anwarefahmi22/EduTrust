# EduTrust — DEV Dependency Audit v1.7

**Scope:** Frontend + backend dependency audit after DEV Vertical Slice #8 (Refund Operations) implementation
...
**VS8 introduced zero new dependencies** — the refund consoles (admin "Refunds (operational)" section; parent "Refund status" button) reuse the existing Next.js 14 + React 18 stack and the existing `lib/api.ts` client (`apiGet`/`apiPost` with `Idempotency-Key` support already present); `requirements.txt`, `package.json`, and `package-lock.json` are byte-identical to pre-VS8 (verified by diff). The finding set is identical to Dependency Audits v1.2–v1.6.
...
Secrets scan (tracked files, excluding test fixtures): no private keys, cloud key material, or hardcoded credential patterns found. No runtime/generated artifacts (`node_modules`, `.next`) tracked or staged (both gitignored; verified by `git check-ignore`).

---

# 2. DEP-001 — Next.js advisory set (carried from v1.2–v1.6, unchanged)
...
| Advisories observed | Image Optimizer DoS via remotePatterns (GHSA-9g9p-9gw9-jx7f); RSC HTTP deserialization DoS (high, GHSA-h25m-26qc-wcjf); HTTP request smuggling in rewrites (GHSA-ggv3-7p47-pfv8); unbounded next/image disk cache (moderate, GHSA-3x4c-7xq6-v6v3); Server Components DoS advisories (GHSA-q4gf-8mx6-v5v3, GHSA-8h8q-6873-q5fj, and related) |
...
# 3. DEP-002 — PostCSS advisories via next (carried from v1.2–v1.6, unchanged)
...
```text
VS8_INTRODUCED_NEW_FINDINGS: NO
NPM_AUDIT_FIX_FORCE_RUN: NO
SECRET_SCAN: CLEAN (tracked files)
GENERATED_ARTIFACTS_TRACKED: NO (node_modules/.next gitignored)
DEPENDENCY_SET: BYTE-IDENTICAL TO PRE-VS8
```
