# EduTrust — DEV Dependency Audit v1.8

**Scope:** Frontend + backend dependency audit after DEV Vertical Slice #9 (Dispute Resolution) implementation
**Commands:** `npm audit --json` in `frontend/` (auditReportVersion 2); `pip check` in the backend venv (Python 3.11.2)
**Status:** FINDINGS — ACCEPT TEMPORARILY IN DEV; FIX BEFORE STAGING/PRODUCTION (finding set unchanged from v1.7)
**Chain:** v1.7 (after VS8) → **v1.8 (after VS9)**
...
```text
affected packages: 2
  next@14.2.35     (direct dependency, package.json "next": "^14.2.0")   severity: high (aggregate)
  postcss@8.4.31   (transitive via next)                                 severity: high (aggregate)
advisories observed: 25 (high: 10, moderate: 13, low: 2)
```

**VS9 introduced zero new dependencies** — the dispute consoles (admin "Disputes (operational)" section; parent/teacher dispute detail views) reuse the existing Next.js 14 + React 18 stack and the existing `lib/api.ts` client (`apiGet`/`apiPost` with `Idempotency-Key` support already present). `requirements.txt`, `package.json`, and `package-lock.json` are byte-identical to pre-VS9 (`git diff b73d8ce` empty — verified). The affected-package set is identical to Dependency Audits v1.2–v1.7; the advisory count delta vs v1.7's snapshot reflects the upstream advisory database updating between runs, not any change in this repository.
...
Secrets scan (tracked files + the VS9 diff): no private keys, cloud key material, or hardcoded credential patterns found. No runtime/generated artifacts (`node_modules`, `.next`) tracked or staged (both gitignored; `.next` verified with `git check-ignore` after the production build).

---

# 2. DEP-001 — Next.js advisory set (carried from v1.2–v1.7, unchanged)
...
# 3. DEP-002 — PostCSS advisories via next (carried from v1.2–v1.7, unchanged)
...
```text
VS9_INTRODUCED_NEW_FINDINGS: NO
NPM_AUDIT_FIX_FORCE_RUN: NO
SECRET_SCAN: CLEAN (tracked files + VS9 diff)
GENERATED_ARTIFACTS_TRACKED: NO (node_modules/.next gitignored)
DEPENDENCY_SET: BYTE-IDENTICAL TO PRE-VS9 (== b73d8ce)
```
